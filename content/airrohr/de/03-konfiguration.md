...
2. Die Station versucht, eine Verbindung mit dem konfigurierten WiFi herzustellen. Falls dies nicht funktioniert, öffnet der Sensor einen Access Point mit dem Namen `Particulate Matter ID` , `Feinstaubsensor-ID` oder `airRohr-ID`. Die ID ist die **ChipID** (zum Beispiel 13597771). **Bitte notiere dir diese Nummer, da diese für die Registrierung benötigen wird.**
...
1. Unter 'Konfigurieren' geben deine SSID (Name deines WiFi-Heimnetzwerks), den Netzwerksicherheitsschlüssel (unter Windows) oder das WiFi-Passwort ein.

2. Für Feinstaubsensoren (SDS011) sind gemäß diesem Handbuch keine weiteren Änderungen erforderlich
