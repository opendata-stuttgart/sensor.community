            ...
            <div class="relative bg-white p-6 grid grid-cols-2">
                {#each langauges as lang}
                    <a on:click={menuToggle} href="{`${lang}/${pathWithoutLang}`}"
                       class="uppercase inline-block p-2 text-gray-700 font-semibold rounded hover:bg-gray-100 hover:text-gray-700"
                       class:selected="{lang === {lang} ? 'selected' : ''}">{flag(lang)} {lang}</a>
                {/each}
            </div>
            <div class="hidden rounded-b-lg md:block bg-gray-50 p-8 bg-gray-100">
