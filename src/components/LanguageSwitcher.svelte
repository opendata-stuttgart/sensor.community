...
<div class="relative">
    ...
    <div class="absolute right-0 mt-2 w-24 bg-white rounded-lg shadow-xl border { menu.open ? 'block' : 'hidden' }">
        <a on:click={menuToggle} href="{`en/${pathWithoutLang}`}" class="uppercase block px-2 py-1 text-gray-700 font-semibold rounded hover:bg-teal-500 hover:text-white" class:selected="{lang === 'en' ? 'selected' : ''}"> {flag('usa')} en</a>
